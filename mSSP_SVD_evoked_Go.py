...
for Go condition (eye blink topographies)
...
# show figures on screen or not (not sure it this works)
show = False
...
svd_fname = '/group/erp/data/caroline.coutout/MEG/data/SVD_windows_Go.pickle'
f_svd = open(svd_fname, 'w')

# lexical and semantic decision tasks
cond_prefix = ['lex', 'sem']

lat_wins = [[0.5,1.0]] # for eye blink topographies (list of lists)
...
for [ssi, ss] in enumerate(subjs):
    ...
    for [cci, cc] in enumerate(cond_prefix):
        ...
        evoked_go = mne.read_evokeds(evoked_fname, baseline=baseline)

        # get first n_comp SVD components within latency window for channel types separately
        svd_comp, svd_s = mSSP_SVD_on_Evoked(evoked_go[0], lat_wins, n_comp=n_comp)
        ...
        for [twi, tw] in enumerate(lat_wins):
            ...
            fig_fname = fig_path + '/' + 'SVDcomps_' + ss[0] + '_' + cc + '_' + win_str + '_Go.pdf'
            with PdfPages(fig_fname) as pdf:
                pdftitle = 'Go - ' + win_str
                pdf.attach_note(pdftitle)
                svd_evoked[win_str].plot_topomap(ch_type='mag', show=show)
                pdf.savefig()
                plt.close()
                plt.bar(range(n_comp), svd_var[twi]['mag'])
                pdf.savefig()
                plt.close()
                svd_evoked[win_str].plot_topomap(ch_type='grad', show=show)
                pdf.savefig()
                plt.close()
                plt.bar(range(n_comp), svd_var[twi]['grad'])
                pdf.savefig()
                plt.close()
                svd_evoked[win_str].plot_topomap(ch_type='eeg', show=show)
                pdf.savefig()
                plt.close()
